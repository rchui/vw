...
def describe_window_functions():
    ...
    def describe_row_number():
        ...
        def test_row_number_with_order_by():
            ...
            q = ref("orders").select(col("id"), F.row_number().over(order_by=[col("created_at").desc()]))
            result = render(q)
        ...
        def test_row_number_with_partition_by():
            ...
            q = ref("orders").select(col("id"), F.row_number().over(partition_by=[col("customer_id")]).alias("row_num"))
            result = render(q)
            assert result.query == sql(expected_sql)
            assert result.params == {}
    ...
    def describe_rank():
        ...
        def test_rank_with_order_by():
            ...
            q = ref("students").select(col("id"), F.rank().over(order_by=[col("score").desc()]).alias("rank"))
            result = render(q)
            assert result.query == sql(expected_sql)
            assert result.params == {}
    ...
    def describe_ntile():
        ...
        def test_ntile_with_order_by():
            ...
            q = ref("employees").select(col("id"), F.ntile(4).over(order_by=[col("salary").desc()]).alias("quartile"))
            result = render(q)
            assert result.query == sql(expected_sql)
            assert result.params == {}
