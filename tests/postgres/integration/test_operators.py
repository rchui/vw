...
def describe_logical_operators() -> None:
    ...
    def it_renders_complex_logic() -> None:
        ...
        SELECT id
        FROM users
        WHERE ((active) AND (verified)) OR (role = 'admin')
        """
        q = (
            source("users")
            .select(col("id"))
...
def describe_real_world_patterns() -> None:
    def it_builds_filtered_price_query() -> None:
        ...
        FROM products
        WHERE (((price >= 10) AND (price <= 100)) AND (in_stock)) AND (category = 'electronics')
        ORDER BY price ASC
        LIMIT 20
        """
        q = (
    ...
    def it_builds_user_search_with_nulls() -> None:
        ...
        FROM users
        WHERE ((email IS NOT NULL) AND (email LIKE '%@company.com')) OR (role = 'admin')
        ORDER BY created_at DESC
        """
        q = (
            source("users")
