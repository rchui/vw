...
def render(
    ...
    """Render a RowSet or Expression to Snowflake SQL.

    Args:
        obj: A RowSet or Expression to render.
        config: Optional rendering configuration (defaults to Snowflake PYFORMAT style).

    Returns:
        SQL result with query string and parameters dict.

    Example:
        >>> from vw.snowflake import ref, col, param, render
        >>> result = render(ref("users").select(col("id")).where(col("age") >= param("min_age", 18)))
        >>> result.query
        'SELECT id FROM users WHERE age >= %(min_age)s'
        >>> result.params
        {'min_age': 18}
    """
    ctx = RenderContext(config=config or RenderConfig(param_style=ParamStyle.PYFORMAT))
...
def render_state(state: object, ctx: RenderContext) -> str:
    ...
    Raises:
        ...
        RenderError: If an unsupported Snowflake feature is used.
    """
    match state:
        # --- Core Query States ----------------------------------------- #
        case CTE():
...
def render_source(
    ...
    For File: raises RenderError (file sources not supported in Snowflake).
...
def render_values(values_src: Values, ctx: RenderContext) -> str:
    ...
    Returns:
        The SQL string (e.g., "(VALUES (%(v0_0_id)s, %(v0_1_name)s)) AS t(id, name)").
...
def render_statement(stmt: Statement, ctx: RenderContext) -> str:
    ...
    Raises:
        RenderError: If DISTINCT ON is used (not supported in Snowflake).
    """
    parts = []

    # SELECT clause (with optional DISTINCT — but not DISTINCT ON)
    if stmt.columns:
        if stmt.distinct and stmt.distinct.on:
            raise RenderError("DISTINCT ON is not supported in Snowflake; use ROW_NUMBER() window function instead")
        elif stmt.distinct:
    ...
    # Modifiers — FOR UPDATE/FOR SHARE not supported in Snowflake; raise if any are present
    for modifier in stmt.modifiers:
        raise RenderError(f"Statement modifier not supported in Snowflake: {type(modifier).__name__}")
...
def render_parameter(param: Parameter, ctx: RenderContext) -> str:
    ...
    Returns:
        The SQL placeholder string (e.g., "%(age)s" for Snowflake PYFORMAT style).
    """
...
def render_raw_source(raw_source: RawSource, ctx: RenderContext) -> str:
    ...
    Example:
        ...
        renders to: generate_series(1, %(max)s) AS t
    """
    sql = raw_source.sql
